"""
classifier.py  (v10.0 — synonyms 강화 + 짧은 핵심 토큰 지원)

변경사항 vs v9.2:
- SHORT_IMPORTANT 집합: 2글자 핵심 모델명(지피, 조에 등) 매칭 지원
- synonym compact 매칭: 띄어쓰기 무시 비교 추가
- _NOISE 확장: 소재/색상어 더 많이 제거해 token 노이즈 감소
- confidence 임계값 0.6 → 0.65 (오분류 감소)
- 품번 패턴에 Q번대 추가
"""
...
_NOISE_WORDS = {
    ...
    '토뤼옹', '마히나', '캔버스', '카프스킨', '타이가라마', '그래파이트',
    '마카사르', '이클립스', '아주르', '에벤', '아르마냑', '누아르',
    '푸시아', '로즈', '발레린',
}
...
def fetch_meta_from_gist(gist_owner: str, gist_id: str) -> dict:
    ...
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'resell-classifier/10.0'})
...
class SmartClassifier:
    ...
    def classify(self, title: str, content: str = '', category: str = '') -> dict:
        ...
        result = self._match_models(self._all_model_cache, full, compact, core_tok)
        if result:
            return result
...
def main():
    parser = argparse.ArgumentParser(description='Resell Classifier v10.0')
    ...
    print(f'=== Classifier v10.0 시작 === Gist:{args.gist_id[:8]}... / 청크:{args.chunk_idx}')
    if GROQ_API_KEY:
    ...
    for item in items:
        ...
        if res['confidence'] >= 0.65 and res['model_name'] not in ('미분류', ''):
            ...
            # ★ 미분류 항목 저장 (url, title, content, category 포함)
            unclassified.append({
                'title':    title,
