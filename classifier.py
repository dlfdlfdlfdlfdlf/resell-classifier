...
def fetch_chunk_from_gist(gist_owner: str, gist_id: str, chunk_idx: int, max_retry: int = 4) -> list | None:
    """
    Gist raw URL: https://gist.githubusercontent.com/{owner}/{gist_id}/raw/chunk_{idx}.json
    Secret Gist는 URL을 알면 인증 없이 다운로드 가능.
    """
    filename = f'chunk_{chunk_idx}.json'
    url = f'https://gist.githubusercontent.com/{gist_owner}/{gist_id}/raw/{filename}'
    print(f'[Gist] 다운로드 중: {url[:60]}...')
...
def main():
    parser = argparse.ArgumentParser(description='Resell Classify Worker v2')
    parser.add_argument('--gist_id',    required=True, help='GitHub Gist ID')
    parser.add_argument('--gist_owner', required=True, help='Gist 소유자 GitHub 계정명')
    parser.add_argument('--chunk_idx',  type=int, required=True, help='처리할 청크 인덱스')
    args = parser.parse_args()

    print(f'=== Classify Worker v2 시작 ===')
    ...
    print(f'✅ 완료: {matched}/{total}건 분류 ({rate:.1f}%) → {out_file}')
