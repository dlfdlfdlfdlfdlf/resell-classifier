...
# ──────────────────────────────────────────────────────────────
#  Gist에서 정규화 맵 읽어오기 (할머니 전용 Gist)
# ──────────────────────────────────────────────────────────────
GIST_ID = 'd8db7c29d1fefb7ec25e1b60b32dac56' # ← 할머니의 Gist ID로 수정 완료!

def load_normalize_map_from_gist():
    """할머니의 Gist에서 최신 정규화 맵을 다운로드합니다."""
    ...
    try:
        ...
        print(f'[Gist] 맵 로드 실패, 기본 맵 사용: {e}')
        # 인터넷이 안 될 때를 대비한 최소한의 기본 맵
        return {
            '팔레모': '팔레르모',
            '네버플': '네버풀',
            '스피디25': '스피디 25',
